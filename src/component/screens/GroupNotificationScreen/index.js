...
export default class GroupNotificationScreen extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            showModal: false,
            isLoading: false,
            serverDataGroupRequest: [],
            serverDataGroupsJoined: [],
            userDetails: '',
            isSelected: false,
            buttonData: [{ title: 'New for you', isSelected: false }, { title: 'Your Group', isSelected: false }, { title: 'Discover', isSelected: false }, { title: 'Notifications', isSelected: true }, { title: 'Your Feeds', isSelected: false }]
    ...
    getMyGroupNotification = () => {
        const { userDetails } = this.state;

        this.setState({ isLoading: true });

        fetch(GLOBAL.BASE_URL + 'api/CommonAPI/GropJoinRequest',
            {
                method: 'POST',
                headers: {
            ...
            .then((respData) => {
                ...
                this.setState({ isLoading: false, serverDataGroupRequest: respData.response }, () => {
                    console.log('serverDataGroupRequest-->>', JSON.stringify(this.state.serverDataGroupRequest))
    ...
    acceptGroupRequest = (Id, type) => {
        const { userDetails } = this.state;

        console.log(Id);
        fetch(GLOBAL.BASE_URL + 'api/CommonAPI/AcceptGroupRequest',
            {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
            ...
            .then((respData) => {
                console.log('respData UpdateGroupRequest', JSON.stringify(respData));
                if (respData.response.StatusCode == 1) {
                    const FilterData = this.state.serverDataGroupRequest.filter(item => item.TRID != Id);
                    this.setState({ serverDataGroupRequest: FilterData });
                }
            }).catch((error) => {
                console.log(error);
                ...
                <View style={{ marginStart: 25 }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 8 }}>
                        <Button
                            ...
                            onPress={() => this.acceptGroupRequest(item.TRID, 'Accepted')}
                        ...
                        <Button
                            ...
                            onPress={() => this.acceptGroupRequest(item.TRID, 'Rejected')}
    ...
    render() {
        const { isLoading, dataSource, isSeeAll, showModal, serverDataGroupRequest, isSelected, buttonData } = this.state;
        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: '#FBFBF9' }}>
                ...
                <TitleHeader title={false} title={'NOTIFICATIONS'} tapOnBack={() => this.props.navigation.goBack()}
                    ...
                    customTitleStyle={{ marginStart: 0, width: width / 3 }}
                ...
                {serverDataGroupRequest.length > 0 ? <FlatList
                    style={{ marginBottom: 12, marginStart: 20, marginEnd: 20 }}
                    data={serverDataGroupRequest}
                    extraData={serverDataGroupRequest}
